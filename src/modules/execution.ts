...
export class ExecutionModule {
  ...
  evaluate(state: MarketState): void {
    ...
    if (this.activeByMarket.get(state.conditionId)) return;
    if (!this.strategy.shouldExecute(state)) return;
  ...
  private executeDryRun(conditionId: string, order: Order): void {
    ...
    setTimeout(() => {
      ...
      console.log(`[Execution][DryRun] FILLED ${order.id}`);
    }, 2_000);
  }
  ...
  private async executeLive(conditionId: string, order: Order): Promise<void> {
    try {
      ...
      console.log(`[Execution][Live] Ordem enviada: ${remoteId}`);
      this.pollOrderStatus(conditionId, order, remoteId);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      console.error(`[Execution][Live] Falha: ${message}`);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (status === 'FILLED' || status === 'MATCHED') {
          ...
          this.filledOrders.push(order);
          this.activeByMarket.set(conditionId, false);
          console.log(`[Execution][Live] FILLED: ${remoteId}`);
        } else if (status === 'CANCELLED' || status === 'REJECTED') {
          clearInterval(interval);
          order.status = 'CANCELLED';
          this.activeOrders.delete(order.id);
          this.activeByMarket.set(conditionId, false);
          console.log(`[Execution][Live] ${status}: ${remoteId}`);
        }
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : String(err);
        console.error(`[Execution] Poll error: ${message}`);
      }
    }, 5_000);
  }
